...
for name in names:
    ...
    frequencies = []
    fitnesses = []
    fitfile = output_directory + name + "_delweakweak_true_aa_fitness.txt"
    freqfile = output_directory + name + "_delweakweak_true_codon_frequencies.txt"
    dndsfile = output_directory + name + "_delweakweak_true_dnds.csv"
    selcfile = output_directory + name + "_delweakweak_true_selcoeffs.csv"
    
    origfit = np.loadtxt( output_directory + name + "_delstrong_true_aa_fitness.txt" )
    for row in origfit:
    
        aafit = np.copy(row)
        aafit[aafit <= -10] = -6 # This still means the unobserved have the lowest fitness (since lowest is around -5.4 for non-lethal), but the change is only weakly deleterious now.
