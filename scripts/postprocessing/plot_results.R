...
ggsave(paste0(plot_directory, "jsd_maintext.pdf"), fig1, width = 7, height = 6) 
...
ggsave(paste0(plot_directory, "repr_dnds_scatter.pdf"), fig2, width=10, height=2)
...
ggsave(paste0(plot_directory, "r2_bias.pdf"), fig3, width=7, height=6)
...
fig4b <- ggplot(jsd.true.slope.p, aes(x = method, y = slope, shape=sig)) + geom_point(position = position_jitter(w = 0.28)) + scale_shape_manual(values=c(1,19)) + geom_hline(yintercept=0) + theme(legend.position="none") + xlab("Inference Method") + ylab("Slope")+ theme(axis.text.x = element_text(size = 11), axis.text.y = element_text(size = 10), axis.title = element_text(size = 13))

## Full figure 4
fig4 <- plot_grid(fig4a, fig4b, nrow=2, labels=c("A", "B"), label_size=17)
ggsave(paste0(plot_directory, "jsd_dnds.pdf"), fig4, width=9, height=4)
...
##fig5a
spread.dnds %>% filter(dataset == repr_sim, method == "nopenal") %>% ggplot(aes(x = true, y = dnds)) + geom_point() + geom_abline(slope = 1, intercept = 0, color="red") + facet_grid(~del) + xlab("True dN/dS") + ylab("swMutSel dN/dS") + scale_x_continuous(limits = c(0,0.8)) + scale_y_continuous(limits=c(0, 0.8)) -> fig5a_1
sc %>% filter(method %in% c("true","nopenal")) %>% ggplot(aes(x = binnedcoeff, fill = method)) + geom_density() + facet_grid(~del) + scale_fill_manual(values =c("grey40", rgb(1, 1, 0, 0.4))) + ylab("Density") + xlab("Scaled Selection Coefficients") + theme(strip.text = element_blank()) -> fig5a_2
fig5a <- plot_grid(fig5a_1, fig5a_2, nrow=2)

##fig5b
spread.dnds %>% filter(dataset == repr_sim, method == "phylobayes") %>% ggplot(aes(x = true, y = dnds)) + geom_point() + geom_abline(slope = 1, intercept = 0, color="red") + facet_grid(~del) + xlab("True dN/dS") + ylab("pbMutSel dN/dS") + scale_x_continuous(limits = c(0,0.8)) + scale_y_continuous(limits=c(0, 0.8)) -> fig5b_1
sc %>% filter(method %in% c("true","phylobayes")) %>% ggplot(aes(x = binnedcoeff, fill = method)) + geom_density() + facet_grid(~del) + scale_fill_manual(values = c("grey40", rgb(1, 1, 0, 0.4))) + ylab("Density") + xlab("Scaled Selection Coefficients") + theme(strip.text = element_blank()) -> fig5b_2
fig5b <- plot_grid(fig5b_1, fig5b_2, nrow=2)

## Figure5
fig5 <- plot_grid(fig5a, fig5b, nrow=1, labels=c("A", "B"), label_size = 15, scale=0.95)
ggsave(paste0(plot_directory, "dnds_sc_weakstrong_raw.pdf"), fig5, width=10, height=5)
