...
ggsave(paste0(plot_directory, "figureS1.pdf"), mean.jsd.weak.boxplots, width = 7.5, height = 4) 
...
ggsave(paste0(plot_directory, "figure2.pdf"), fig2, width=10, height=2)
...
## Figure 4B
# note: p-values are multiplied by num_compare, as Bonferroni correction.
alpha <- 0.01*8 # 8 is the number of methods
...
jsd.true.slope.p$method <- factor(jsd.true.slope.p$method, levels = c("nopenal", "mvn100", "mvn10", "mvn1", "d0.01", "d0.1", "d1.0", "phylobayes"), labels = c("No penalty", "mvn100", "mvn10", "mvn1", "d0.01", "d0.1", "d1.0", "pbMutSel"))
fig4b <- ggplot(jsd.true.slope.p, aes(x = method, y = slope, color=sig)) + geom_point(position = position_jitter(w = 0.4)) + scale_color_manual(values=c("grey70", "black")) + geom_hline(yintercept=0) + theme(legend.position="none") + xlab("Inference Method") + ylab("Slope")+ theme(axis.text.x = element_text(size = 11), axis.text.y = element_text(size = 10), axis.title = element_text(size = 13))


fig4 <- plot_grid(fig4a, fig4b, nrow=2, labels=c("A", "B"), label_size=17)
ggsave(paste0(plot_directory, "figure4.pdf"), fig4, width=8, height=4)
